...
float x,y;
        XYProjOnPath(cgp.x,cgp.y,_xpts,_ypts,_npts,&x,&y,&_clkidx,&r);
        cgp.x=x;
        cgp.y=y;
		if((r>0)&&(r<1)){
...
//
// -----------
bool bToolJustification::edit(void* prm){
_bTrace_("bToolJustification::edit",true);
long    result=0;
...
int		idx;
float	r,x,y;
	XYProjOnPath(cgp.x,cgp.y,_xpts,_ypts,_npts,&x,&y,&idx,&r);
    cgp.x=x;
    cgp.y=y;
	if(idx==0){
		return(0);
