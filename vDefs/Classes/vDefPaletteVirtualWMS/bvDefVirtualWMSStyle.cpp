...
char	fullurl[2048];
	...
	if(_srs>0){
		sprintf(fullurl,
				"http://%s?Service=WMS&Version=1.1.0&Layers=%s&Styles=%s&Format=%s&SRS=EPSG:%d",
				_url,_layer,_style,_fmt,_srs);
		//"http://%s?Service=WMS&Version=1.1.0&Request=GetMap&Layers=%s&Styles=%s&Format=%s&SRS=EPSG:%d",
	}
	else{
		sprintf(fullurl,
				"http://%s?Service=WMS&Version=1.1.0&Layers=%s&Styles=%s&Format=%s",
				_url,_layer,_style,_fmt);
		//"http://%s?Service=WMS&Version=1.1.0&Layers=%s&Styles=%s&Format=%s",
	}
