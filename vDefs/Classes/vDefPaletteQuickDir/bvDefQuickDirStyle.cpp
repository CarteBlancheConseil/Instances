//----------------------------------------------------------------------------
// File : bvDefQuickDirStyle.cpp
// Project : MacMap
// Purpose : C++ source file : Arrow (direction) vdef style class
...
// Constructeur
// ------------
bvDefQuickDirStyle	::bvDefQuickDirStyle(bGenericMacMapApp* gapp,
										 bGenericType* gtp,
										 int clss)
...
// Destructeur
// -----------
bvDefQuickDirStyle::~bvDefQuickDirStyle(){
}

// ---------------------------------------------------------------------------
// 
// ------------
bool bvDefQuickDirStyle::load(bGenericXMLBaseElement* root){
...
// for new subtypes
	...
	if(elt=_gapp->classMgr()->NthElement(_gstl,1,"size")){
		...
		if(chld=_gapp->classMgr()->NthElement(elt,1,"field")){
			chld->getvalue(val);
			_fsz=_gtp->fields()->get_index(val);
		}
	...
	if(elt=_gapp->classMgr()->NthElement(_gstl,1,"arrow")){
		if(chld=_gapp->classMgr()->NthElement(elt,1,"field")){
			chld->getvalue(val);
			_fdir=_gtp->fields()->get_index(val);
		}
		else{
			_fdir=kOBJ_Dir_;
		}
...
// 
// ------------
bool bvDefQuickDirStyle::dump(bArray& arr, int indent){
char	nm[256];
	if((_sz==0)&&(_fsz==0)){
		return(true);
	}
...
// Geometry
	...
	if(_fsz){
		...
		_gtp->fields()->get_name(_fsz,nm);
		add_cdesc(arr,indent+4,"field",nm);
		add_ddesc(arr,indent+4,"value",_sz,2);
	}
	else{
		add_ddesc(arr,indent+3,"size",_sz,2);
	}
	...
	add_cdesc(arr,indent+3,"arrow","");
	_gtp->fields()->get_name(_fdir,nm);
	add_cdesc(arr,indent+4,"field",nm);
