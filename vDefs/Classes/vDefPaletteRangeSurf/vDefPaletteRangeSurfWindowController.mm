//----------------------------------------------------------------------------
// File : vDefPaletteRangeSurfWindowController.mm
// Project : MacMap
// Purpose : Objective C++ source file : Choropleth vdef window controller
...
#import "vDefPaletteRangeSurfWindowController.h"
#import "bvDefPaletteRangeSurf.h"
#import "bvDefRangeSurfStyle.h"
#import "CocoaStuff.h"
...
// 
// ------------
@implementation vDefPaletteRangeSurfPanel
...
// 
// ------------
@implementation vDefPaletteRangeSurfWindowController

// ---------------------------------------------------------------------------
// 
// ------------
-(id)init{
_bTrace_("[vDefPaletteRangeSurfWindowController init]",true);
	self=[super init];
	if(self){
		_last_click=-1;
	}
...
// ------------
-(void)dealloc{
_bTrace_("[vDefPaletteRangeSurfWindowController dealloc]",true);
_tm_((void*)self);
	[super dealloc];
}
...
// ------------
-(void)awakeFromNib{
_bTrace_("[vDefPaletteRangeSurfWindowController awakeFromNib]",true);
	[super awakeFromNib];
...
// ------------
-(void)close{
_bTrace_("[vDefPaletteRangeSurfWindowController close]",true);
_tm_((void*)self);
	[super close];
}
...
_bTrace_("[vDefPaletteRangeSurfWindowController doPutWidth]",true);
...
_bTrace_("[vDefPaletteRangeSurfWindowController doPutStrokeColor]",true);
bvDefRangeSurfStyle*	stl;
...
_bTrace_("[vDefPaletteRangeSurfWindowController doPutSmoothing]",true);
bvDefRangeSurfStyle*	stl;
...
_bTrace_("[vDefPaletteRangeSurfWindowController doChooseColorimetry]",true);
bvDefRangeSurfStyle*	stl;	
...
_bTrace_("[vDefPaletteRangeSurfWindowController doChooseFillKind]",true);	
bvDefRangeSurfStyle*	stl;
...
//				((bvDefPaletteRangeSurf*)_ext)->limits().get(_last_click+1,&stl->_vmin);
//				((bvDefPaletteRangeSurf*)_ext)->limits().get(_last_click+2,&stl->_vmax);
				((bvDefPaletteRangeSurf*)_ext)->limits().get(i,&stl->_vmin);
				((bvDefPaletteRangeSurf*)_ext)->limits().get(i+1,&stl->_vmax);
				stl->_visible=true;
			}
			else{
				stl->_visible=false;
...
_bTrace_("[vDefPaletteRangeSurfWindowController doChooseFillColor]",true);
bvDefRangeSurfStyle*	stl;
...
_bTrace_("[vDefPaletteRangeSurfWindowController doChooseFillPattern]",true);
...
_bTrace_("[vDefPaletteRangeSurfWindowController doChooseFillFromColor]",true);
bvDefRangeSurfStyle*	stl;
...
_bTrace_("[vDefPaletteRangeSurfWindowController doChooseFillToColor]",true);
bvDefRangeSurfStyle*	stl;
...
_bTrace_("[vDefPaletteRangeSurfWindowController doPutFillAlpha]",true);
bvDefRangeSurfStyle*	stl;
...
// -----------
-(void)plot2Intf{
_bTrace_("[vDefPaletteRangeSurfWindowController plot2Intf]",true);
	[super plot2Intf];

bvDefRangeSurfStyle*	stl=NULL;
	for(int i=1;i<=_ext->runs().count();i++){
		_ext->runs().get(i,&stl);
		if(stl->_visible){
			break;
...
CGRect					shape;
	...
	for(long i=1;i<=_ext->runs().count();i++){
		...
		shape=CGRectInset(hir,stl->_width+2.0,stl->_width+2.0);
...
CGPoint				pts[3];
					...
					pts[1].x=pts[0].x;
					pts[1].y=pts[0].y+shape.size.height;
					pts[2].x=pts[1].x+shape.size.width;
					pts[2].y=pts[1].y;
					...
					pts[0].x=shape.origin.x;
					pts[0].y=shape.origin.y;
					pts[1].x=pts[0].x+shape.size.width;
					pts[1].y=pts[0].y;
					pts[2].x=pts[1].x;
					pts[2].y=pts[1].y+shape.size.height;
					CGContextAddLines(ctx,pts,3);
					CGContextClosePath(ctx);
					CGContextDrawPath(ctx,kCGPathFill);
...
void* initializeCocoa(void* gapp, 
					  void* ext) {
vDefPaletteRangeSurfWindowController	*controller;
NSAutoreleasePool						*localPool;
	
    localPool=[[NSAutoreleasePool alloc] init];   
    controller=[[vDefPaletteRangeSurfWindowController alloc] init];
