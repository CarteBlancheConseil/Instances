...
//
// -----------
bool bXMapShortCuts::nsevent(void* nsevt){
bool	res=false;
    if(!_first){
...
user_ext_data	edt;
    ...
    else if([evt type]==NSEventTypeKeyDown){
        key=[evt keyCode];
        tmp=[evt modifierFlags];
        modi=0;
        if(tmp&NSEventModifierFlagShift){
            modi|=shiftKey;
        }
        if(tmp&NSEventModifierFlagControl){
            modi|=controlKey;
        }
        if(tmp&NSEventModifierFlagOption){
            modi|=optionKey;
        }
        if(tmp&NSEventModifierFlagCommand){
            modi|=cmdKey;
        }
