...
long                bXMapSelectorCriterion::_cid=0;
...
// 
// -----------
CFStringRef bXMapSelectorCriterion::get_string(CFBundleRef bndl, bool last){
_bTrace_("bXMapSelectorCriterion::get_string",false);
CFStringRef	cfs;
char		knd[256]="????";
char		intfname[256]="????";
char		opname[256]="????";
...
_tm_("ext name for returned "+intfname+" for "+i);
...
char	cntname[256]="????";
        switch(_cntop){
            case kAndElement:
                b_message_string("and",cntname,bndl,0);
                break;
            case kOrElement:
                b_message_string("or",cntname,bndl,0);
                break;
            case kXOrElement:
                b_message_string("xor",cntname,bndl,0);
                break;
            default:
                break;
        }
        cfs=CFStringCreateWithFormat(kCFAllocatorDefault,NULL,CFSTR("%s %s %s %s %s"),knd,intfname,opname,_valvalue,cntname);
    }
    else{
...
CFStringRef	cfs=get_string(bndl,false);
...
// Ne marche pas, c'est toujours le dernier passé ne correspond pas forcément au parent
// => faire une pile des parents, affecter le dernier, puis décrémenter ?
            if(!strcmp(name,"and")){
                p->cntop=kAndElement;
                p->stackop->add(&p->cntop);
                return(true);
            }
            if(!strcmp(name,"or")){
                p->cntop=kOrElement;
			...
			if(	(p->targetkind!=0)		&&
				(p->targetvalue[0]!=0)	&&
				(p->opsgn!=0)			){
                p->stackop->get(p->stackop->count(),&p->cntop);
                p->stackop->rmv(p->stackop->count());
