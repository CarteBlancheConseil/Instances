...
//
// ------------
void bXMapNetCleanOptions::open(int* flags){
    bStdXMap::open(flags);
...
//
// ------------
void bXMapNetCleanOptions::close(){
    bStdXMap::close();
//    write_p();
//    _nodes.reset();
//    _edges.reset();
...
//
// -----------
bool bXMapNetCleanOptions::event(EventRef evt){
...
//NetCleanOptionscfg_prm*	cp;
//NetCleanOptionsact_prm*	ap;
//bool				b;
//    
//    switch(msg){
...
// Std call
// ------------
void bXMapNetCleanOptions::process_network(bool log){
_bTrace_("bXMapNetCleanOptions::process_network(bool)",true);
bGenericType*	tp;
bool			badf=false;
...
bArray			edges(sizeof(bGenericGeoElement*));
    ...
    b_message_string(kXMapNetCleanOptionsControlField,name,getbundle(),0);
...
bEventLog	log(_gapp,this);
        if(badf){
            b_message_string(kXMapNetCleanOptionsAskAddControlField,name,getbundle(),0);
...
// Event call
// ------------
void bXMapNetCleanOptions::process_network(bArray& nodes, bArray& edges){
_bTrace_("bXMapNetCleanOptions::process_network(bArray&,bArray&)",true);
bGenericType*	tp;
it_prm			iter;
int				fields[_gapp->typesMgr()->count()];
NetCleanOptionsact_prm	bkprm;
...
// Common call
// ------------
void bXMapNetCleanOptions::process_network(NetCleanOptionsact_prm* prm,
                                    int* fields,
                                    bArray& nodes,
                                    bArray& edges){
_bTrace_("bXMapNetCleanOptions::process_network(NetCleanOptionsact_prm,int*,bArray&,bArray&)",true);
    if(_cfg_prm.tnod==NULL){
        return;
    }
...
//
// -----------
void bXMapNetCleanOptions::check_events(){
//_bTrace_("bXMapNetCleanOptions::check_events",true);
...
//_tm_("creator:"+&cs);
        if(	(evt->creator()==sign)	||
            (evt->is_undo())		||
            (evt->is_redo())		){
//_tm_("creator=NetCleanOptions, Undo, or Redo");
            continue;
        }
...
//
// ------------
int bXMapNetCleanOptions::create_nodes(bArray& loop, bArray& added, int* fields, bool report){
_bTrace_("bXMapNetCleanOptions::create_nodes",true);
    if(!_cfg_prm.tnod){
        return(0);
    }
...
char				ttl[__MESSAGE_STRING_LENGTH_MAX__];
    ...
    b_message_string(kXMapNetCleanOptionsMessageID,ttl,getbundle(),0);
    b_message_string(kXMapNetCleanOptionsNodeCreationMessage,msg,getbundle(),0);
...
//
// ------------
int bXMapNetCleanOptions::cut_edges(bArray& loop, bArray& added, int* fields, bool report){
_bTrace_("bXMapNetCleanOptions::cut_edges",true);
bGenericGeoElement	*oa,*ob,*op;
bGenericType		*tp;
...
char				ttl[__MESSAGE_STRING_LENGTH_MAX__];
    ...
    b_message_string(kXMapNetCleanOptionsMessageID,ttl,getbundle(),0);
    b_message_string(kXMapNetCleanOptionsCutEdgesMessage,msg,getbundle(),0);
bProgressWait	wt(ttl,msg,true,true,loop.count());
    for(long i=1;i<=loop.count();i++){
        if(!wt.set_progress(i)){
            break;
...
//
// ------------
int bXMapNetCleanOptions::join_on_nodes(bArray& loop, int* fields, bool report){
_bTrace_("bXMapNetCleanOptions::join_on_nodes",true);
...
char				ttl[__MESSAGE_STRING_LENGTH_MAX__];
    ...
    b_message_string(kXMapNetCleanOptionsMessageID,ttl,getbundle(),0);
    b_message_string(kXMapNetCleanOptionsJoinOnNodesMessage,msg,getbundle(),0);
bProgressWait	wt(ttl,msg,true,true,loop.count());
    for(long i=1;i<=loop.count();i++){
        if(!wt.set_progress(i)){
            break;
...
//
// ------------
int bXMapNetCleanOptions::join_on_edges(bArray& loop, int* fields, bool report){
_bTrace_("bXMapNetCleanOptions::join_on_edges",true);
...
char				ttl[__MESSAGE_STRING_LENGTH_MAX__];
    ...
    b_message_string(kXMapNetCleanOptionsMessageID,ttl,getbundle(),0);
    b_message_string(kXMapNetCleanOptionsJoinOnEdgesMessage,msg,getbundle(),0);
bProgressWait	wt(ttl,msg,true,true,loop.count());
    for(long i=1;i<=loop.count();i++){
        if(!wt.set_progress(i)){
            break;
...
//
// ------------
int bXMapNetCleanOptions::check_nodes(bArray& loop, int* fields){
_bTrace_("bXMapNetCleanOptions::check_nodes",true);
...
char				ttl[__MESSAGE_STRING_LENGTH_MAX__];
    ...
    b_message_string(kXMapNetCleanOptionsMessageID,ttl,getbundle(),0);
    b_message_string(kXMapNetCleanOptionsCheckNodesMessage,msg,getbundle(),0);
bProgressWait	wt(ttl,msg,true,true,loop.count());
    for(long i=1;i<=loop.count();i++){
        if(!wt.set_progress(i)){
            break;
...
//
// ------------
int bXMapNetCleanOptions::check_edges(bArray& loop, int* fields){
_bTrace_("bXMapNetCleanOptions::check_edges",true);
bGenericGeoElement	*oa;
ivertices			*vsa;
...
char				ttl[__MESSAGE_STRING_LENGTH_MAX__];
    ...
    b_message_string(kXMapNetCleanOptionsMessageID,ttl,getbundle(),0);
    b_message_string(kXMapNetCleanOptionsCheckEdgesMessage,msg,getbundle(),0);
bProgressWait	wt(ttl,msg,true,true,loop.count());
    for(long i=1;i<=loop.count();i++){
        if(!wt.set_progress(i)){
            break;
...
//
// ------------
void bXMapNetCleanOptions::get_nodes(i2dvertex* vx, bArray& arr){
//_bTrace_("bXMapNetCleanOptions::get_nodes",true);
    arr.reset();
it_prm					iter;
bGenericGeoElement*		o;
...
//
// ------------
void bXMapNetCleanOptions::get_bridges(i2dvertex* vx, bArray& arr){
//_bTrace_("bXMapNetCleanOptions::get_bridges",true);
    arr.reset();
...
//
// ------------
void bXMapNetCleanOptions::get_edges(ivx_rect* vr, bArray& arr){
//_bTrace_("bXMapNetCleanOptions::get_edges(ivx_rect* vr, bArray& arr)",true);
    arr.reset();
it_prm	iter;
nc_prm	prm;
...
//
// ------------
void bXMapNetCleanOptions::get_edges(i2dvertex* vx, bArray& arr, bool by_end){
//_bTrace_("bXMapNetCleanOptions::get_edges(i2dvertex* vx, bArray& arr, bool by_end)",true);
    arr.reset();
it_prm		iter;
ivx_rect    vr={vx->v,vx->h,vx->v,vx->h};
nc_prm		prm;
...
// 
// ------------
void bXMapNetCleanOptions::init_fields(NetCleanOptionsact_prm* prm){
_bTrace_("bXMapNetCleanOptions::init_fields",true);
it_prm      iter;
init_prm	ip;
char		name[__MESSAGE_STRING_LENGTH_MAX__];
    ...
    if(prm->check_nodes){
        for(long i=1;i<=_nitr.count();i++){
            ...
            if(!iter.field){
                ...
                iter.field=iter.tp->fields()->get_index(name);
                if(iter.field){
                    iter.tp->fields()->set_default_value(iter.field,(void*)kXMapNetCleanOptionsControlFieldDefault);
                }
            }
            if(iter.field){
                ip.field=iter.field;
...
// 
// ------------
void bXMapNetCleanOptions::read_p(){
_bTrace_("bXMapNetCleanOptions::read_p",true);
...
bool					b=_gapp->document()->readTree(&root,GetSignature(this),kXMapNetCleanOptionsConfig);
char					val[_values_length_max_];
bGenericType*			tp;
long					i,idx;
    
    if((!b)||(!root)){
_tw_("((!b)||(!root)) for "+kXMapNetCleanOptionsConfig);
        return;
    }
...
// 
// ------------
void bXMapNetCleanOptions::write_p(){
bGenericXMLBaseElement* root=make_p();
    if(!root){
        return;
    }
    if(!_gapp->document()->writeTree(root,GetSignature(this),kXMapNetCleanOptionsConfig)){
        return;
    }
...
// 
// ------------
bGenericXMLBaseElement* bXMapNetCleanOptions::make_p(){
...
init_prm*	p=(init_prm*)prm;
    geo->setValue(p->field,kXMapNetCleanOptionsControlFieldDefault);
    return(0);
}
...
char	val[32];
    o->getValue(fld,val);
    if(strlen(val)!=8){
        strcpy(val,kXMapNetCleanOptionsControlFieldDefault);
    }
    val[flg-1]='1';
