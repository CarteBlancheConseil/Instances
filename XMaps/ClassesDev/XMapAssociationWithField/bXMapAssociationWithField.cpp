...
bXMapAssociationWithField	::bXMapAssociationWithField(bGenericXMLBaseElement* elt, bGenericMacMapApp* gapp, CFBundleRef bndl) 
	...
	setclassname("associationwithfield2");
...
void bXMapAssociationWithField::open(int* flags){	
	...
	_prm.lmax=10000;
	_prm.needhier=true;
	_prm.field=kOBJ_Name_;
}

// ---------------------------------------------------------------------------
//
// -----------
bool bXMapAssociationWithField::edit(void* prm){
_bTrace_("bXMapAssociationWithField::edit()",true);
long                result=0;
...
//
// -----------
bGenericType* bXMapAssociationWithField::get_type(){
    return _gapp->typesMgr()->get(SelectionIsMonoType(_gapp));
}
...
bEventLog	log(_gapp,this);
	associate(_prm.needhier?kOBJ_SubType_:0,
              _prm.field/*-1+kOBJ_Name_*/,
              Measure_d2i(_gapp,_prm.lmin),
              Measure_d2i(_gapp,_prm.lmax),
              false);
	log.close();
	return(true);
}
